...
We propose a fixed version of the {\em OpenSecureChannel} sub-protocol using
...
standards~\cite{MLD09,opcua_part2,opcua_part4,opcua_part6}, to ensure
the secrecy of derived keys in security mode \sms.  All occurrences of
$N_{C}$ are replaced by $\left\{\mbox{$N_{C}$}\right\}_{pk(S)}$ in
message 3 and all occurrences of $N_{S}$ in message 4 by
$\left\{\mbox{$N_{S}$}\right\}_{pk(C)}$.  Thus all the messages are
signed in security mode \sms{} but only the nonces are encrypted.
...
\begin{flushleft}
    $\begin{array}{lrllll}
        3 & C & \rightarrow & S & : & \text{\sm{\oscreq, pk(C), $N_{C}$, {\bf pk(S)}}{pk(S)}{sk(C)}}\\
        4 & S & \rightarrow & C & : & \text{\sm{\oscres, $N_{S}$, ST, TTL, {\bf pk(C)}}{pk(C)}{sk(S)}}\\
    \end{array}$
\end{flushleft}
}
%\vspace{-.5em}

 We also use \proverif to check the security of the protocol with all
 ...
 \smse.  We checked the counter-measures separately and to confirm
 that both are required (explicitly specifying the receiver of a
 message in its signature fixes attacks found on authentication and
 key wrapping ensures the secrecy of the keys).
