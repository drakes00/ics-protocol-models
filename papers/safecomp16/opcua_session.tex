...
Figure \ref{fig:session_se}.  In message 1, $C$ sends a nonce as a challenge
to $S$ with {\em CSReq} meaning {\em CreateSessionRequest}.  In
...
that the session is created with {\em ASRes} for {\em
  ActivateSessionResponse} and $N_{S2}$ a fresh nonce as a challenge that
...
As this protocol involves login and passwords, we assume that $C$ uses a
...
equation: \texttt{verifyCreds(pk(S), Login(pk(C)), Passwd(sk(C), pk(S))) =
  true}.
  %where \texttt{x} the private key of the server and \texttt{y}
%the private key of the client (not revealed to the server).
  It allows
...
Results without key wrapping (thus with keys leaked in security mode \sms, cf.
Table \ref{tab:secure_channel_results}) are
